...
function ShoeItem({ shoe, shoeID }) {
  ...
  const handleToggleFavorite = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/Account/cookieGetById`,
        { credentials: "include" }
      );
      ...
      if (!favoriteResponse.ok)
        throw new Error("Lỗi khi cập nhật danh sách yêu thích");
    ...
    <li className="col item">
      <div className="image">
        <div className="url_img">
          <img
            src={`${process.env.REACT_APP_API_URL}/${shoe.imageUrl}`}
            alt={shoe.name}
          />
        </div>
        <button
      ...
      <a onClick={() => navigate(`/product/${shoe.id || shoeID}`)}>
        ...
        <div className="item-detail">
          ...
          <p>Pricing {shoe.price.toLocaleString("vi-VN")}đ</p>
        </div>
      </a>
      <div className="btn-heart" onClick={handleToggleFavorite}>
        <i
          className={`fa-solid fa-heart ${
            isShoeFavorite ? "active-heart" : ""
          }`}
        />
      </div>
    </li>
