...
const Update = (props) => {
      ...
      if (props.slug === "product") {
        ...
        setShoeData((prevData) => {
          const updatedSizes = prevData.sizes.map((sizeObj) => {
            const foundSize = res.shoeDetails.find(
              (detail) => detail.size === sizeObj.size
            );
            return {
              size: sizeObj.size,
  ...
  const handleInputChange = (event) => {
    ...
    if (props.slug === "discount") {
      if (
        name === "amount" &&
        (discount.percentage || discount.maximumDiscount)
      ) {
        props.setOpenToastMessage(true);
        props.setTypeToastMessage("warning");
        props.setTitleToastMessage(
          "Only discount by percentage or amount can be entered."
        );
        return;
      }
      if (
        (name === "percentage" || name === "maximumDiscount") &&
        discount.amount
      ) {
        props.setOpenToastMessage(true);
        props.setTypeToastMessage("warning");
  ...
  const handleSizeChange = (index, field) => (event) => {
    ...
    if (/^\d*$/.test(value) || value === "") {
      // Cho phép chỉ số và trường trống
  ...
  const handleSubmit = async (e) => {
    ...
    if (props.slug === "product") {
      const updateShoe = {
        ...
        additionalImages:
          Array.isArray(shoeData.additionalImages) &&
    ...
    if (props.slug === "user") {
      const updatedUser = {
        ...user,
        gender:
          user.gender === "Male" ? true : user.gender === "Female" ? false : "", // Gán giá trị true/false dựa trên giới tính thực tế
    ...
    if (props.slug === "discount") {
      ...
      const formatDiscount = {
        ...
        percentage:
          discount.percentage === "" ? 0 : parseInt(discount.percentage),
        quantity: parseInt(discount.quantity),
        maximumDiscount:
          discount.maximumDiscount === ""
            ? 0
            : parseInt(discount.maximumDiscount),
  ...
  return (
    <div className="add">
      <div className="add-modal">
        ...
        <form
          className={props.slug === "order" ? "form-order" : ""}
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    >
                      ...
                      <TextField
                        ...
                        value={
                          shoeData[input.field_price]
                            ? shoeData.price.replace(
                                /\B(?=(\d{3})+(?!\d))/g,
                                ","
                              )
                            : ""
                ...
                {input.type === "selectOnly" && input.field !== "gender" && (
                  <>
                    ...
                    <FormControl sx={{ width: 300 }}>
                      <InputLabel id={`label-${input.field}`}>
                        {input.field}
                      </InputLabel>
                      <Select
                  ...
                  <>
                    ...
                    <FormControl sx={{ width: 300 }}>
                      ...
                      <Select
                        ...
                        name={input.field}
                        value={
                          props.slug === "product" ? genderUi : user?.gender
                        }
                        onChange={(e) => handleSelectOnlyChange(e, input.field)}
                        input={<OutlinedInput label={input.label} />}
                        MenuProps={MenuProps}
                  ...
                  <>
                    ...
                    <FormControl sx={{ width: 300 }}>
                      ...
                      <Select
                        ...
                        name={input.field}
                        value={
                          shoeData[input.field].map(
                            (item) => item[input.few]
                          ) || []
                        }
                        onChange={(e) =>
                          handleSelectMultipleChange(e, input.field, input.few)
                        }
                        input={<OutlinedInput label={input.label} />}
                        MenuProps={MenuProps}
                      >
                  ...
                  <>
                    ...
                    <div className="file-container">
                      <div className="file-item">
                        <label
                          htmlFor="mainImage"
                          className="preview preview-main-image"
                        >
                          {shoeData.imageUrl ? (
                            <img
                          ...
                          <span>
                            {shoeData.mainImage ? "" : "Upload Main Image Here"}
                          </span>
                        </label>
                        <input
                          type="file"
                      ...
                      <div className="file-item">
                        <label
                          ...
                          className="preview preview-additional-images"
                        >
                          ...
                          <span>
                            {shoeData.additionalImages.length > 0
                              ? `${shoeData.additionalImages.length} Files Uploaded`
                              : "Upload Sub Images"}
                          </span>
                        </label>
                        <input
